...
/// <summary>
/// Wem der Gateway die <c>X-Forwarded-*</c>-Header glaubt (<c>MCPMCP_TRUSTED_PROXIES</c>).
/// <para>
...
internal static class ForwardedProxyOptions
{
    ...
    public static bool TryCreate(string? configured, out ForwardedHeadersOptions options)
    {
        ...
        foreach (var entry in configured.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries))
        {
            ...
            else
            {
                throw new InvalidOperationException(
                    $"MCPMCP_TRUSTED_PROXIES enthält '{entry}' — das ist weder eine IP-Adresse noch "
        ...
        {
            throw new InvalidOperationException(
                "MCPMCP_TRUSTED_PROXIES ist gesetzt, ergibt aber keine Herkunft. Leer lassen schaltet "
