...
/// <summary>
/// Holt eine Beschreibung von einer URL — größenbegrenzt und mit Zielprüfung.
/// <para>
/// Ein Gateway, das eine vom Administrator genannte URL abruft, ist ohne diese Prüfung ein Werkzeug,
/// um <b>interne</b> Dienste zu erreichen: Cloud-Metadaten unter <c>169.254.169.254</c>, ein
...
internal static class SpecFetcher
{
        ...
        Uri location, bool allowPrivateTargets, TimeSpan timeout, CancellationToken ct)
    {
        ...
        if (location.IsFile)
        {
            ...
            {
                throw new OpenRpcImportException($"Dokument überschreitet {MaxBytes / (1024 * 1024)} MB.");
            }
        ...
        {
            await EnsureTargetAllowedAsync(current, allowPrivateTargets, ct).ConfigureAwait(false);
            ...
            if (IsRedirect(response.StatusCode))
            {
                var next = response.Headers.Location
                    ?? throw new OpenRpcImportException("Weiterleitung ohne Ziel.");
            ...
            return await ReadLimitedAsync(response, ct).ConfigureAwait(false);
        }

        throw new OpenRpcImportException($"Mehr als {MaxRedirects} Weiterleitungen — abgebrochen.");
    }
        ...
        Uri target, bool allowPrivateTargets, CancellationToken ct)
    {
        ...
        else
        {
            ...
            {
                throw new OpenRpcImportException(
                    $"Host '{target.Host}' ließ sich nicht auflösen: {exception.Message}");
            }
        }
