...
- **Status:** Abgeschlossen und abgenommen (2026-07-26). Alle Arbeitspakete WP1–WP7 geliefert, alle
  ...
  (Run [30195270952](https://github.com/LupusMalusDeviant/bifrost/actions/runs/30195270952)).
...
**Ergebnis:** Ein `bifrost-wasi-host`-Binary, das den Vertrag spricht; Vertrag als versioniertes Schema dokumentiert.
...
| WP6.1 Rest (Readiness, Lifecycle, Capability-Flags) | Der Handshake nennt jetzt **Capability-Flags** (`cancellation`, `concurrency`, `drain`, `readiness`, `persistentInstances`, `resources`, `secrets`, `diskCache` — und `streams: false`, die Zurückstellung als Aussage statt als Lücke). Fehlt dem Client ein Pflichtfeature, kommt der Upstream **nicht hoch**, mit Nennung des fehlenden Namens. `health` unterscheidet **Bereitschaft von Leben**: `ready` plus `phase` (`handshake` → `negotiated` → `ready` → `draining`) plus laufende Aufrufe. Neu ist `drain`: keine neuen Aufrufe mehr (eigener Code `draining`), Warten auf die laufenden, und die Antwort sagt mit `idle`, ob es sauber war. Das Gateway drainiert vor `shutdown`; ohne diesen Schritt schnitt `shutdown` seit v4 laufende Aufrufe ab. Tests: 4 Rust-Tests (Readiness ≠ Leben, Drain weist ab und ist in `health` sichtbar, Drain wartet wirklich — mit einem gestaffelten Reader, weil alle Frames auf einmal die Threads aneinander vorbeirasen lassen, Shutdown wartet nicht auf einen rechnenden Guest), 2 Stub-Tests, 1 Real-Host-Test über die ganze Kette |
...
- **WP6.1 ist abgeschlossen.** Namen, Schemata, Discovery, Invoke, Fehlersemantik, Cancellation
  (Vertrag v4), Readiness, Lifecycle-Phasen und Capability-Flags stehen. Was von ADR-0016 **nicht**
  in diesem Plan liegt und offen bleibt: Packaging und Installation von Connector-Paketen,
  signierte Manifeste, Update/Rollback in Quarantäne und die Vertrauensstufen für
  Drittanbieter-Connectoren. Der WASI-Host erfüllt den Vertrag; er ist nicht die Paketverwaltung.
...
| Host unter Supervisor, Win+Linux, non-root; **CI grün auf beiden** | **erfüllt** (2026-07-26, Run [30195270952](https://github.com/LupusMalusDeviant/bifrost/actions/runs/30195270952) auf `9165405`) — `build-test` und `wasi-component-spike` je auf `ubuntu-latest` **und** `windows-latest` grün, dazu der `docker`-Job: Host im Image, non-root (UID-Prüfung), Image < 300 MB, arm64 baut, Compose-Beispiele gültig, `/healthz` antwortet. Auf Linux erzwingt CI `BIFROST_REQUIRE_POSTGRES=1`, in den Vertragstests `BIFROST_REQUIRE_WASI_HOST=1` — ein Skip wäre dort ein Fehlschlag. Übersprungen bleiben nur `benchmark` und `isolation-comparison`, beide bewusst an `schedule`/`workflow_dispatch` gebunden (geteilte Runner sind keine Referenz-Hardware) |
...
- **Packaging** (WP7.1): **entschieden und gebaut** — ein Image mit .NET **und** Rust-Host
  (`/usr/local/bin/bifrost-wasi-host`), damit ein Image genau einen Vertragsstand trägt. Belegt auf
  amd64 und arm64.
