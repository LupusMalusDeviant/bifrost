...
/// <para>
/// Ohne Runtime werden die Tests übersprungen; <c>MCPMCP_REQUIRE_CONTAINER=1</c> erzwingt sie
...
public sealed class ContainerIsolationE2ETests
{
    ...
    {
        var required = Environment.GetEnvironmentVariable("MCPMCP_REQUIRE_CONTAINER") is "1" or "true";
        ...
        if (!available)
        {
            Assert.SkipUnless(required, "Keine Container-Runtime erreichbar — Docker starten oder MCPMCP_REQUIRE_CONTAINER setzen.");
            Assert.Fail("MCPMCP_REQUIRE_CONTAINER ist gesetzt, aber keine Container-Runtime erreichbar.");
        }
    }
