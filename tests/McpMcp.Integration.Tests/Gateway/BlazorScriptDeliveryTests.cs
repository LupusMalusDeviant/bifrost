...
public class BlazorScriptDeliveryTests
{
    ...
    {
        var directory = Path.GetDirectoryName(typeof(McpMcp.Server.ForwardedProxyOptions).Assembly.Location);
        return directory is null
            ? null
            : Directory.EnumerateFiles(directory, "*.staticwebassets.endpoints.json")
                .FirstOrDefault(f => Path.GetFileName(f).StartsWith("McpMcp.Server.", StringComparison.Ordinal));
    }
}
