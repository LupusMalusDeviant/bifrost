...
internal sealed class TestDbContextFactory : IDbContextFactory<BifrostDbContext>
{
    private readonly DbContextOptions<BifrostDbContext> _options;

    public TestDbContextFactory(DbContextOptions<BifrostDbContext> options) => _options = options;
...
public abstract class PersistenceTestsBase : IAsyncLifetime
{
    ...
    internal IDbContextFactory<BifrostDbContext> Factory { get; private set; } = null!;

    protected IDataProtectionProvider DataProtection { get; } = new EphemeralDataProtectionProvider();

    protected abstract Task<DbContextOptions<BifrostDbContext>?> CreateOptionsAsync();

    /// <summary>Liefert eine zusätzliche, noch uninitialisierte Datenbank — für die Migrations-/Upgrade-Tests.</summary>
    protected abstract Task<DbContextOptions<BifrostDbContext>> CreateFreshOptionsAsync(string name);
...
.Should().BeNull();
    ...
    public async Task Recovery_commands_restore_ui_and_agent_access()
    {
        ...
        var created = await Bifrost.Server.AdminCommands.ResetUiAdminAsync(users, username, TestContext.Current.CancellationToken);
        ...
        var reset = await Bifrost.Server.AdminCommands.ResetUiAdminAsync(users, username, TestContext.Current.CancellationToken);
        ...
        var recovery = await Bifrost.Server.AdminCommands.IssueBootstrapKeyAsync(rbac, keys, TestContext.Current.CancellationToken);
    ...
    {
        MarkSkippedIfUnavailable();
        IDbContextFactory<BifrostDbContext> factory = new TestDbContextFactory(await CreateFreshOptionsAsync("fresh"));
    ...
    {
        MarkSkippedIfUnavailable();
        IDbContextFactory<BifrostDbContext> factory = new TestDbContextFactory(await CreateFreshOptionsAsync("legacy"));
