...
public class ApprovalFlowTests
{
    ...
    public async Task After_approval_the_same_call_runs_exactly_once()
    {
        ...
        var fp = Bifrost.Core.Approvals.ApprovalFingerprint.Compute(admin, _w.Echo, redacted);
    ...
    public async Task Approval_does_not_transfer_to_a_different_argument()
    {
        ...
        store.Approve(admin, _w.Echo, Bifrost.Core.Approvals.ApprovalFingerprint.Compute(admin, _w.Echo, redacted));
