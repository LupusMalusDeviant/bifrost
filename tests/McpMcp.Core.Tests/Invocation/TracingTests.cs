...
public sealed class TracingTests
{
        ...
        Func<InvokerTestWorld, Task> act)
    {
        ...
        lock (captured)
        {
            var own = captured.FirstOrDefault(a =>
                a.OperationName == "mcpmcp.tool_call"
                && a.GetTagItem("mcpmcp.tool") is string tool
    ...
    public async Task A_successful_call_produces_a_span_with_tool_status_and_origin()
    {
        ...
        var call = activities.Should().ContainSingle(a => a.OperationName == "mcpmcp.tool_call").Subject;
        call.GetTagItem("mcpmcp.tool").Should().Be(world.Echo.Value);
        call.GetTagItem("mcpmcp.server").Should().Be(world.Slug);
        call.GetTagItem("mcpmcp.status").Should().Be(nameof(InvocationStatus.Success));
        call.GetTagItem("mcpmcp.origin").Should().NotBeNull();
    ...
    public async Task The_upstream_call_gets_its_own_child_span()
    {
        ...
        var call = activities.Single(a => a.OperationName == "mcpmcp.tool_call");
        var upstream = activities.Should().ContainSingle(a => a.OperationName == "mcpmcp.upstream_call").Subject;
        ...
        upstream.GetTagItem("mcpmcp.upstream_tool").Should().Be("echo",
    ...
    public async Task A_denied_call_is_marked_as_an_error_span()
    {
        ...
        var call = activities.Single(a => a.OperationName == "mcpmcp.tool_call");
        call.GetTagItem("mcpmcp.status").Should().Be(nameof(InvocationStatus.Denied));
        call.Status.Should().Be(ActivityStatusCode.Error);
        activities.Should().NotContain(a => a.OperationName == "mcpmcp.upstream_call",
